...
import { resetLimit } from './users.js'; // Import fungsi intinya
...
async function restaringServer(sock) {
  try {
    logWithTime('System', 'Menyiapkan penjadwalan harian jam 12 malam (Reset & Restart)');

    const jobName = `daily-midnight-task`;
    
    // Jalan setiap jam 00:00
    schedule.scheduleJob(jobName, '46 17 * * *', async () => {
      try {
        logWithTime('System', 'Memulai tugas tengah malam...');

        // --- STEP 1: RESET DATA ---
        logWithTime('System', 'Mereset Limit dan Money users...');
        await resetLimit(); // Panggil fungsi dari lib/users.js
        
        // --- STEP 2: RESTART SERVER (Optional) ---
        // Jika kamu ingin bot benar-benar fresh setelah reset
        logWithTime('System', 'Restarting system otomatis ...');
        setTimeout(async () => {
    await restaringAction();
}, 5000); // Jeda 5 detik agar I/O database selesai dulu

      } catch (error) {
        logWithTime('System', `Error daily task: ${error.message}`);
        console.error('Error daily task:', error);
      }
    });

    logWithTime('System', 'Penjadwalan harian berhasil diatur');
  } catch (error) {
    logWithTime('System', `Error in daily task setup: ${error.message}`);
  }
}
